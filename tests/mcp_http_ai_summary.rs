...
fn http_ai_summary_json_and_presets() {
    ...
    let payload = resp.unwrap().json::<serde_json::Value>().unwrap_or(serde_json::json!({}));
    assert_eq!(payload.get("status").and_then(|v| v.as_str()).unwrap_or(""), "ok");
    let js = payload.get("json").cloned().unwrap_or(serde_json::json!({}));

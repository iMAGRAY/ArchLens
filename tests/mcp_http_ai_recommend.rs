...
fn http_ai_recommend_with_summary() {
    ...
    let sum_json = sum_resp.unwrap().json::<serde_json::Value>().unwrap_or(serde_json::json!({}));
    let summary = sum_json.get("json").cloned().unwrap_or(serde_json::json!({}));
    ...
    let rec_json = rec_resp.unwrap().json::<serde_json::Value>().unwrap_or(serde_json::json!({}));

    assert_eq!(rec_json.get("status").and_then(|v| v.as_str()).unwrap_or(""), "ok", "status ok");
    let recs = rec_json.get("recommendations").and_then(|v| v.as_array()).cloned().unwrap_or_default();
    assert!(!recs.is_empty(), "should return at least one recommendation");

...
impl DiffAnalyzer {
    ...
    pub fn analyze_diff(&self, current: &CapsuleGraph, previous: &CapsuleGraph) -> Result<DiffAnalysis> {
        ...
        let recommendations = self.generate_recommendations(&changes, &metrics_diff, &quality_trend)?;
    ...
    fn analyze_component_changes(&self, current: &CapsuleGraph, previous: &CapsuleGraph, changes: &mut Vec<ArchitectureChange>) -> Result<()> {
        let current_components: HashMap<String, &Capsule> = current.capsules.values()
            .map(|c| (c.name.clone(), c))
            .collect();
        
        let previous_components: HashMap<String, &Capsule> = previous.capsules.values()
            .map(|c| (c.name.clone(), c))
            .collect();
        ...
        for (name, capsule) in &current_components {
            if !previous_components.contains_key(name) {
                changes.push(ArchitectureChange {
                    ...
                    description: format!("Добавлен новый компонент '{}' типа {:?}", name, capsule.capsule_type),
        ...
        for (name, capsule) in &previous_components {
            if !current_components.contains_key(name) {
                changes.push(ArchitectureChange {
                    ...
                    component: name.clone(),
                    description: format!("Удален компонент '{}' типа {:?}", name, capsule.capsule_type),
                    impact: self.impact_calculator.calculate_remove_impact(capsule, previous),
    ...
    fn analyze_component_modifications(&self, current: &Capsule, previous: &Capsule, changes: &mut Vec<ArchitectureChange>) -> Result<()> {
        // Изменение сложности
        ...
        if complexity_delta.abs() > self.change_threshold {
            ...
            changes.push(ArchitectureChange {
                ...
                impact: if complexity_delta > 5.0 { ChangeImpact::Major } else { ChangeImpact::Minor },
        ...
        if current.warnings.len() != previous.warnings.len() {
            ...
            changes.push(ArchitectureChange {
                ...
                impact: if warning_delta > 0 { ChangeImpact::Quality } else { ChangeImpact::Quality },
                related_components: Vec::new(),
            });
        }
    ...
    fn analyze_relation_changes(&self, current: &CapsuleGraph, previous: &CapsuleGraph, changes: &mut Vec<ArchitectureChange>) -> Result<()> {
        let current_relations: HashSet<(String, String)> = current.relations.iter()
            .filter_map(|r| {
        ...
        let previous_relations: HashSet<(String, String)> = previous.relations.iter()
            .filter_map(|r| {
    ...
    fn calculate_metrics_diff(&self, current: &CapsuleGraph, previous: &CapsuleGraph) -> Result<MetricsDiff> {
        Ok(MetricsDiff {
            complexity_delta: current.metrics.complexity_average - previous.metrics.complexity_average,
            ...
            component_count_delta: current.metrics.total_capsules as i32 - previous.metrics.total_capsules as i32,
            relation_count_delta: current.metrics.total_relations as i32 - previous.metrics.total_relations as i32,
    ...
    fn calculate_quality_trend(&self, metrics_diff: &MetricsDiff, changes: &[ArchitectureChange]) -> Result<QualityTrend> {
        let mut quality_score = 0.0;
        
        // Анализ метрик
        if metrics_diff.complexity_delta < 0.0 { quality_score += 1.0; }
        if metrics_diff.coupling_delta < 0.0 { quality_score += 1.0; }
        if metrics_diff.cohesion_delta > 0.0 { quality_score += 1.0; }
        ...
        // Анализ изменений
        let breaking_changes = changes.iter().filter(|c| c.impact == ChangeImpact::Breaking).count();
        let quality_changes = changes.iter().filter(|c| c.impact == ChangeImpact::Quality).count();
    ...
    /// Генерация рекомендаций
    fn generate_recommendations(&self, changes: &[ArchitectureChange], metrics_diff: &MetricsDiff, quality_trend: &QualityTrend) -> Result<Vec<String>> {
        let mut recommendations = Vec::new();
        
        // Рекомендации по метрикам
        if metrics_diff.complexity_delta > 1.0 {
            recommendations.push("Рассмотрите рефакторинг для снижения сложности компонентов".to_string());
        }
        
        if metrics_diff.coupling_delta > 0.1 {
            recommendations.push("Высокая связанность - используйте инверсию зависимостей".to_string());
        }
        
        if metrics_diff.cohesion_delta < -0.1 {
            recommendations.push("Низкая сплоченность - группируйте связанную функциональность".to_string());
        }
        
        // Рекомендации по изменениям
        let breaking_changes = changes.iter().filter(|c| c.impact == ChangeImpact::Breaking).count();
        if breaking_changes > 0 {
            recommendations.push("Обнаружены критические изменения - обновите документацию и тесты".to_string());
        }
        
        let new_dependencies = changes.iter().filter(|c| c.change_type == ChangeType::NewDependency).count();
        if new_dependencies > 5 {
            recommendations.push("Много новых зависимостей - проверьте архитектурную целостность".to_string());
        }
        
        // Рекомендации по тренду качества
        match quality_trend {
            QualityTrend::Degrading => {
                recommendations.push("Качество кода ухудшается - необходим технический аудит".to_string());
            }
            QualityTrend::Improving => {
                recommendations.push("Качество кода улучшается - продолжайте в том же духе".to_string());
            }
            QualityTrend::Mixed => {
                recommendations.push("Смешанные изменения - сфокусируйтесь на критических областях".to_string());
            }
            QualityTrend::Stable => {
                recommendations.push("Архитектура стабильна - можно добавлять новую функциональность".to_string());
            }
        }
    ...
    fn generate_summary(&self, changes: &[ArchitectureChange], metrics_diff: &MetricsDiff, quality_trend: &QualityTrend) -> Result<String> {
        let total_changes = changes.len();
        let added_components = changes.iter().filter(|c| c.change_type == ChangeType::Added).count();
        let removed_components = changes.iter().filter(|c| c.change_type == ChangeType::Removed).count();
        let modified_components = changes.iter().filter(|c| matches!(c.change_type, ChangeType::Modified | ChangeType::ComplexityIncrease | ChangeType::ComplexityDecrease)).count();
        
        let summary = format!(
            "Обнаружено {} изменений: {} добавлено, {} удалено, {} изменено. \
            Сложность: {:+.1}, Связанность: {:+.2}, Сплоченность: {:+.2}. \
            Тренд качества: {:?}.",
            total_changes, added_components, removed_components, modified_components,
            metrics_diff.complexity_delta, metrics_diff.coupling_delta, metrics_diff.cohesion_delta,
            quality_trend
        );
...
impl ImpactCalculator {
    ...
    fn calculate_remove_impact(&self, capsule: &Capsule, graph: &CapsuleGraph) -> ChangeImpact {
        let dependents_count = graph.relations.iter()

...
impl ArgParser {
            ...
            match arg.as_str() {
                "--max-depth" => {
                    ...
                    if let Some(depth_str) = self.current() {
                        max_depth = Some(depth_str.parse()
    ...
    fn parse_diagram(&mut self) -> Result<CliCommand, String> {
        ...
        let diagram_type_str = self.current()
        ...
        let diagram_type = match diagram_type_str.as_str() {
            ...
            "dot" => DiagramType::Dot,
            "svg" => DiagramType::Svg,
            _ => return Err(format!("Неподдерживаемый тип диаграммы: {}", diagram_type_str)),
        };

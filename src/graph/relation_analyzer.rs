...
impl RelationAnalyzer {
    ...
    fn create_import_patterns() -> HashMap<FileType, Vec<Regex>> {
        ...
        patterns.insert(
            FileType::Go,
            vec![Regex::new(r#"import\s+(?:\(\s*)?["']([^"']+)["']"#).unwrap()],
        );
    ...
    fn create_export_patterns() -> HashMap<FileType, Vec<Regex>> {
        ...
        patterns.insert(
            FileType::Java,
            vec![Regex::new(r"public\s+(?:class|interface|enum|static\s+\w+)\s+(\w+)").unwrap()],
        );

        // C++ exports
        patterns.insert(
            FileType::Cpp,
            vec![Regex::new(r"(?:class|struct|namespace|extern)\s+(\w+)").unwrap()],
        );

        // Go exports
        patterns.insert(
            FileType::Go,
            vec![Regex::new(r"(?:func|type|var|const)\s+([A-Z]\w*)").unwrap()],
        );
    ...
    pub fn build_advanced_relations(&self, capsules: &[Capsule]) -> Result<Vec<CapsuleRelation>> {
        ...
        for capsule in capsules {
            ...
            for other_capsule in capsules {
                if capsule.id != other_capsule.id {
                    if let Some(strength) =
                        self.calculate_file_relation_strength(capsule, other_capsule)
                    {
            ...
            for other_capsule in capsules {
                if capsule.id != other_capsule.id {
                    if let Some(strength) =
                        self.calculate_layer_relation_strength(capsule, other_capsule)
                    {
                        if strength > self.relation_strength_threshold {
                            relations.push(CapsuleRelation {
                                from_id: capsule.id,
            ...
            if let Ok(content) = std::fs::read_to_string(&capsule.file_path) {
                if let Some(semantic_relations) =
                    self.analyze_semantic_relations(capsule, &content, capsules)
                {
    ...
    fn calculate_layer_relation_strength(
        &self,
        capsule1: &Capsule,
        capsule2: &Capsule,
    ) -> Option<f32> {
        ...
        for other_capsule in all_capsules {
            ...
            if let Ok(other_content) = std::fs::read_to_string(&other_capsule.file_path) {
                ...
                let other_exports = self
                    .extract_exports(&other_content, &other_file_type)
                    .unwrap_or_default();

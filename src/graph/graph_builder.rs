...
use crate::types::*;
use crate::graph::{CycleDetector, RelationAnalyzer, MetricsCalculator};
...
impl CapsuleGraphBuilder {
    ...
    pub fn build_graph(&mut self, capsules: &[Capsule]) -> Result<CapsuleGraph> {
        ...
        let updated_capsules = self.relation_analyzer.update_capsule_dependencies(&capsule_map, &relations)?;
        
        // Calculate graph metrics
        let metrics = self.metrics_calculator.calculate_advanced_metrics(&updated_capsules, &relations)?;
        ...
        if !cycles.is_empty() {
            self.cycle_detector.add_cycle_warnings(&mut graph, &cycles)?;
        }
    ...
    pub fn analyze_graph(&mut self, graph: &CapsuleGraph) -> Result<GraphAnalysis> {
        ...
        let coupling_metrics = self.metrics_calculator.calculate_coupling_metrics(&graph.capsules, &graph.relations);
        let cohesion_metrics = self.metrics_calculator.calculate_cohesion_metrics(&graph.capsules, &graph.relations);
        let complexity_distribution = self.metrics_calculator.calculate_complexity_distribution(&graph.capsules);
    ...
    pub fn validate_graph(&mut self, graph: &CapsuleGraph) -> Result<Vec<GraphValidationIssue>> {
        ...
        for cycle in cycles {
            issues.push(GraphValidationIssue {
                ...
                suggestion: "Break the cycle through dependency inversion or interface extraction".to_string(),
            });
        }
        ...
        if !orphaned.is_empty() {
            issues.push(GraphValidationIssue {
                ...
                suggestion: "Consider connecting these capsules or removing them if unused".to_string(),
            });
        }
        ...
        if coupling_metrics.average_instability > 0.8 {
            issues.push(GraphValidationIssue {
                ...
                severity: Priority::Medium,
                description: format!("High coupling detected (instability: {:.2})", coupling_metrics.average_instability),
                affected_capsules: Vec::new(),
                suggestion: "Reduce coupling by introducing abstractions and interfaces".to_string(),
            });
        }
        ...
        if cohesion_metrics.average_layer_cohesion < 0.3 {
            issues.push(GraphValidationIssue {
                ...
                severity: Priority::Medium,
                description: format!("Low cohesion detected (cohesion: {:.2})", cohesion_metrics.average_layer_cohesion),
                affected_capsules: Vec::new(),
    ...
    pub fn optimize_graph(&mut self, graph: &mut CapsuleGraph) -> Result<Vec<OptimizationResult>> {
        ...
        graph.metrics = self.metrics_calculator.calculate_advanced_metrics(&graph.capsules, &graph.relations)?;

...
impl ContentAnalyzer {
    ...
    fn calculate_duplication_ratio(&self, content: &str) -> f32 {
        ...
        let duplicated_lines = line_counts
            .values()
    ...
    fn calculate_complexity_factor(&self, content: &str, file_type: &FileType) -> f32 {
        let complexity_keywords = match file_type {
            ...
            FileType::JavaScript | FileType::TypeScript => {
                vec!["if", "else", "for", "while", "switch", "try", "catch"]
            }
            FileType::Python => vec![
    ...
    fn count_functions(&self, content: &str, file_type: &FileType) -> usize {
        match file_type {
            ...
            FileType::JavaScript | FileType::TypeScript => {
                let pattern =
                    Regex::new(r"function\s+\w+|const\s+\w+\s*=\s*\(|let\s+\w+\s*=\s*\(").unwrap();
                pattern.find_iter(content).count()
            }
            FileType::Python => {
    ...
    fn create_test_patterns() -> HashMap<FileType, Regex> {
        ...
        patterns.insert(
            FileType::JavaScript,
            Regex::new(r"(test|it|describe)\s*\(").unwrap(),
        );
        patterns.insert(
            FileType::TypeScript,
            Regex::new(r"(test|it|describe)\s*\(").unwrap(),
        ...
        patterns.insert(
            FileType::Python,
            Regex::new(r"def\s+test_\w+|class\s+\w*Test\w*").unwrap(),
        );

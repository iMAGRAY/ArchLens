...
impl CodeSmellDetector {
        ...
        for cap in fn_pattern.captures_iter(content) {
            ...
            if lines_after > threshold {
                smells.push(CodeSmell {
                    ...
                    suggestion: format!(
                        "Разбейте функцию '{}' на несколько более мелких функций",
                        fn_name
                    ),
            ...
            if let Some(impl_match) = impl_regex.find(content) {
                ...
                if impl_lines > threshold {
                    smells.push(CodeSmell {
                        ...
                        suggestion: format!(
                            "Разбейте структуру '{}' на несколько более мелких",
                            struct_name
                        ),
    ...
    fn detect_empty_exception_handling(
        &self,
        content: &str,
        rule: &SmellRule,
    ) -> Result<Vec<CodeSmell>> {
    ...
    fn detect_too_many_comments(&self, content: &str, rule: &SmellRule) -> Result<Vec<CodeSmell>> {
        ...
        if total_lines > 0.0 && comment_lines / total_lines > threshold {
            smells.push(CodeSmell {
                ...
                suggestion: "Упростите код, чтобы уменьшить необходимость в комментариях"
                    .to_string(),
                location: None,
                confidence: 0.6,
            });
    ...
    fn detect_file_specific_smells(
        &self,
        content: &str,
        file_type: FileType,
    ) -> Result<Vec<CodeSmell>> {

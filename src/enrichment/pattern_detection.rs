...
impl PatternDetector {
    ...
    pub fn detect_patterns(&self, content: &str, semantic_links: &[SemanticLink]) -> Result<Vec<ArchitecturalPattern>> {
        ...
        for (pattern_type, rules) in &self.pattern_rules {
            ...
            for rule in rules {
                ...
                if !matches.is_empty() {
                    ...
                    evidence.push(format!("{}: найдено {} совпадений", rule.name, matches.len()));
                }
            }
    ...
    fn calculate_semantic_boost(&self, pattern_type: &PatternType, semantic_links: &[SemanticLink]) -> f32 {
        match pattern_type {
            PatternType::Singleton => {
                // Singleton обычно имеет статические методы
                semantic_links.iter()
                    .filter(|link| link.target_name.contains("instance") || link.target_name.contains("get_instance"))
                    .count() as f32 * 0.2
            },
            PatternType::Factory => {
                // Factory обычно создает объекты
                semantic_links.iter()
                    .filter(|link| link.target_name.contains("create") || link.target_name.contains("new"))
                    .count() as f32 * 0.1
            },
            PatternType::Observer => {
                // Observer имеет методы подписки
                semantic_links.iter()
                    .filter(|link| link.target_name.contains("subscribe") || 
            ...
            PatternType::Repository => {
                ...
                semantic_links.iter()
                    .filter(|link| link.target_name.contains("save") || 
    ...
    fn get_pattern_description(&self, pattern_type: &PatternType) -> String {
        match pattern_type {
            PatternType::Singleton => "Паттерн Singleton обеспечивает создание единственного экземпляра класса".to_string(),
            PatternType::Factory => "Паттерн Factory создает объекты без указания их конкретных классов".to_string(),
            PatternType::Observer => "Паттерн Observer определяет зависимость один-ко-многим между объектами".to_string(),
            PatternType::Strategy => "Паттерн Strategy определяет семейство алгоритмов и делает их взаимозаменяемыми".to_string(),
            PatternType::Command => "Паттерн Command инкапсулирует запрос как объект".to_string(),
            PatternType::Builder => "Паттерн Builder конструирует сложные объекты пошагово".to_string(),
            PatternType::Adapter => "Паттерн Adapter позволяет объектам с несовместимыми интерфейсами работать вместе".to_string(),
            PatternType::Repository => "Паттерн Repository инкапсулирует логику доступа к данным".to_string(),
            PatternType::Service => "Паттерн Service содержит бизнес-логику приложения".to_string(),
            PatternType::Controller => "Паттерн Controller обрабатывает входящие запросы".to_string(),
            PatternType::Decorator => "Паттерн Decorator добавляет новую функциональность объектам".to_string(),
            PatternType::Template => "Паттерн Template Method определяет скелет алгоритма в базовом классе".to_string(),
            PatternType::MVC => "Паттерн MVC разделяет приложение на модель, представление и контроллер".to_string(),
            PatternType::MVP => "Паттерн MVP использует презентер для управления представлением".to_string(),
            PatternType::MVVM => "Паттерн MVVM связывает представление с моделью через view model".to_string(),
            PatternType::Layered => "Слоистая архитектура организует код в логические слои".to_string(),
            PatternType::Microservices => "Микросервисная архитектура разделяет приложение на независимые сервисы".to_string(),
            PatternType::EventDriven => "Событийно-ориентированная архитектура основана на публикации и подписке на события".to_string(),
            PatternType::PipeAndFilter => "Архитектура Pipe and Filter обрабатывает данные через цепочку фильтров".to_string(),
        }
    }
    ...
    fn create_pattern_rules() -> HashMap<PatternType, Vec<PatternRule>> {
        ...
        // Singleton
        rules.insert(PatternType::Singleton, vec![
            PatternRule {
                name: "Приватный конструктор".to_string(),
                pattern: Regex::new(r"private\s+\w+\s*\(").unwrap(),
                weight: 0.4,
                description: "Приватный конструктор для предотвращения создания экземпляров извне".to_string(),
            },
            PatternRule {
                name: "Статический экземпляр".to_string(),
                pattern: Regex::new(r"static\s+.*instance").unwrap(),
                weight: 0.5,
                description: "Статическое поле для хранения единственного экземпляра".to_string(),
            },
            PatternRule {
        ...
        // Factory
        rules.insert(PatternType::Factory, vec![
            PatternRule {
        ...
        // Observer
        rules.insert(PatternType::Observer, vec![
            PatternRule {
        ...
        // Repository
        rules.insert(PatternType::Repository, vec![
            PatternRule {
        ...
        // Service
        rules.insert(PatternType::Service, vec![
            PatternRule {
        ...
        // Controller
        rules.insert(PatternType::Controller, vec![
            PatternRule {
        ...
        // Builder
        rules.insert(PatternType::Builder, vec![
            PatternRule {

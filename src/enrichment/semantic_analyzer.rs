...
impl SemanticAnalyzer {
    ...
    pub fn create_analyzers() -> HashMap<FileType, SemanticAnalyzer> {
        ...
        // C++ analyzer
        analyzers.insert(FileType::Cpp, SemanticAnalyzer {
    ...
    pub fn create_antipattern_detectors() -> Vec<AntipatternDetector> {
        vec![
            ...
            AntipatternDetector {
                ...
                detection_regex: Regex::new(r"(?:fn|function|def)\s+\w+[\s\S]{500,}?(?:\n\s*\}|\n\s*$)").unwrap(),
...
impl SemanticEnricher {
    ...
    fn calculate_quality_metrics(&self, content: &str, semantic_links: &[SemanticLink]) -> Result<QualityMetrics> {
        ...
        let maintainability_index = 171.0 - 5.2 * (lines_of_code.ln()) - 0.23 * (cyclomatic_complexity as f32) - 16.2 * (lines_of_code.ln());
    ...
    fn detect_architectural_patterns(&self, content: &str, _semantic_links: &[SemanticLink]) -> Result<Vec<ArchitecturalPattern>> {
    ...
    fn calculate_technical_debt_ratio(&self, content: &str, _semantic_links: &[SemanticLink]) -> f32 {

...
impl QualityAnalyzer {
    ...
    pub fn analyze_quality(&self, capsule: &Capsule, content: &str) -> Result<QualityAssessment> {
        ...
        let maintainability_index = 171.0 - 5.2 * lines_of_code.ln() - 0.23 * cyclomatic_complexity - 16.2 * lines_of_code.ln();
    ...
    fn calculate_documentation_score(&self, content: &str) -> f32 {
        ...
        let has_function_docs = content.contains("///") || content.contains("/**") || 
                               content.contains("\"\"\"") || content.contains("#");
        
        let has_inline_comments = content.lines().filter(|line| {
            let trimmed = line.trim();
            trimmed.contains("//") && !trimmed.starts_with("//")
        }).count() > 0;
        
        let comment_density = content.lines().filter(|line| {
            let trimmed = line.trim();
        ...
        if has_header_comments { score += 25.0; }
        if has_function_docs { score += 40.0; }
        if has_inline_comments { score += 15.0; }
    ...
    fn calculate_test_coverage_score(&self, content: &str) -> f32 {
        ...
        let has_mocks = content.contains("mock") || 
        ...
        if has_test_functions { score += 40.0; }
        if has_assertions { score += 30.0; }
        if has_mocks { score += 20.0; }
        if has_test_imports { score += 10.0; }
    ...
    fn calculate_maintainability_score(&self, content: &str) -> f32 {
        ...
        if function_count > 0 && lines_count > 0 {
            ...
            if ratio < 50.0 { // Small functions
                score += 20.0;
            } else if ratio > 200.0 { // Large functions
                score -= 20.0;
            }
        }
    ...
    fn calculate_overall_score(
        ...
        (weights.complexity_weight + weights.documentation_weight + 
         weights.test_coverage_weight + weights.maintainability_weight + 
    ...
    fn generate_recommendations(
        ...
        if complexity_score < 60.0 {
            recommendations.push(QualityRecommendation {
                ...
                suggestion: "Break down complex functions into smaller, focused functions".to_string(),
        ...
        if documentation_score < 40.0 {
            recommendations.push(QualityRecommendation {
                ...
                suggestion: "Add documentation comments for public functions and modules".to_string(),

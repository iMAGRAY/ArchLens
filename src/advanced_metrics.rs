...
use crate::types::*;
use crate::enrichment::{SemanticEnricher, QualityAnalyzer};
...
impl AdvancedMetricsCalculator {
    ...
    pub fn calculate_metrics(&self, capsule: &Capsule, content: &str) -> Result<AdvancedMetrics> {
        ...
        let semantic_analysis = self.semantic_enricher.perform_semantic_analysis(capsule, content)?;
    ...
    fn calculate_solid_score(&self, content: &str) -> Result<SOLIDScore> {
        ...
        if content.contains("trait ") || content.contains("interface ") || content.contains("abstract ") {
            ocp_score += 0.2;
        }
    ...
    fn calculate_halstead_metrics(&self, content: &str) -> Result<HalsteadMetrics> {
        ...
        let volume = if vocabulary > 0 { length as f32 * (vocabulary as f32).log2() } else { 0.0 };
        let difficulty = if n2 > 0 { (n1 as f32 / 2.0) * (big_n2 as f32 / n2 as f32) } else { 0.0 };
    ...
    fn count_operands(&self, content: &str) -> HashMap<String, u32> {
        ...
        for word in content.split_whitespace() {
            ...
            if !clean_word.is_empty() && (clean_word.chars().next().unwrap().is_alphabetic() || clean_word.chars().all(|c| c.is_numeric())) {

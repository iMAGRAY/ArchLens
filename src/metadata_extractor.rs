...
impl MetadataExtractor {
    pub fn new() -> Self {
        ...
        patterns.insert("todo".to_string(), vec![
            "TODO".to_string(),
            "todo".to_string(),
            "@todo".to_string(),
        ]);
        
        patterns.insert("fixme".to_string(), vec![
            "FIXME".to_string(),
            "fixme".to_string(),
            "@fixme".to_string(),
        ]);
        
        patterns.insert("deprecated".to_string(), vec![
    ...
    pub fn extract_metadata(&self, element: &ASTElement, _file_path: &Path) -> Result<HashMap<String, String>> {
        ...
        metadata.insert("line_count".to_string(), (element.end_line - element.start_line + 1).to_string());
    ...
    fn extract_type_specific_metadata(&self, element: &ASTElement, metadata: &mut HashMap<String, String>) -> Result<()> {
    ...
    fn extract_function_metadata(&self, element: &ASTElement, metadata: &mut HashMap<String, String>) -> Result<()> {
        metadata.insert("parameters_count".to_string(), element.parameters.len().to_string());
    ...
    fn extract_class_metadata(&self, element: &ASTElement, metadata: &mut HashMap<String, String>) -> Result<()> {
        // Считаем количество детей (методов, полей)
        metadata.insert("children_count".to_string(), element.children.len().to_string());
        ...
        let auto_slogan = match element_type {
            ASTElementType::Function => {
                if name.starts_with("get_") {
                    format!("Получает данные: {}", name.strip_prefix("get_").unwrap_or(name))
                } else if name.starts_with("set_") {
                    format!("Устанавливает: {}", name.strip_prefix("set_").unwrap_or(name))
                    ...
                    format!("Валидирует: {}", name.strip_prefix("validate_").unwrap_or(name))

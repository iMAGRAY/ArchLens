...
- 🔌 **MCP сервер** - интеграция с AI редакторами (Cursor, VSCode, Claude)
...
## 🔌 MCP Сервер для AI Редакторов

ArchLens предоставляет MCP (Model Context Protocol) сервер для интеграции с AI редакторами с возможностями обнаружения архитектурных проблем и ошибок кода.
...
2. **Добавьте в настройки Cursor** (`settings.json`):
   ```json
   {
     "mcp.servers": {
       "archlens": {
         "command": "node",
         "args": ["/path/to/Archinalysator/mcp/archlens_mcp_server.cjs"],
         "cwd": "/path/to/Archinalysator/mcp"
       }
     }
   }
   ```

3. **Перезапустите Cursor** и используйте команды:
   - "Найди архитектурные проблемы в проекте"
   - "Проанализируй код на наличие God Objects и циклических зависимостей"
   - "Выполни анализ SOLID принципов"
   - "Покажи code smells и антипаттерны"
   - "Создай диаграмму с отображением проблемных связей"

### 🔍 Возможности обнаружения проблем через MCP
